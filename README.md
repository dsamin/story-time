...
![No data collected](https://img.shields.io/badge/No-data%20collected-A9C8E0?style=flat-square&labelColor=4A4A4A)
![No way to fail](https://img.shields.io/badge/No-way%20to%20fail-C9A8D8?style=flat-square&labelColor=4A4A4A)
![Fully offline](https://img.shields.io/badge/Fully-offline-B7C99E?style=flat-square&labelColor=4A4A4A)

> **No ads · No in-app purchases · No data collected · No way to fail · Fully on-device.**
...
- **Story Shelf (home).** Wordless grid of story tiles, each showing its cast. Audio names each tile on tap. A parent corner sits behind a press-and-hold gate.
- **Story Player.** The illustrated story with word-by-word highlighting, a large replay control, and tap-a-word-to-hear-it. This is the visual centerpiece.
- **Question Card.** The story illustration shrinks to a banner; below it, a spoken question and 2–4 large picture-answer choices. Errorless feedback animates in place.
- **Sequencing Board.** 2–3 shuffled picture cards and labeled drop zones (first / next / last, shown as 1·2·3 dots + position, not words). Drag-and-drop with generous targets; then the assembled animation replays.
- **End Card.** A soft, quiet resting screen — no nag, no score — with a single tap back to the shelf.
- **Parent Settings (gated).** Behind a press-and-hold-then-drag gate: voice selection, reduce-motion, which stories are active, and a peek at what the review service thinks needs another pass. All text here is for the adult.
...
- **Audio-first, wordless navigation.** Every instruction is spoken; nothing the child must do requires reading. The only readable text is behind the parent gate.
- **Reuse the cast, reuse the art.** Characters, pictures, and audio come from the **shared chassis** the slate already has — this app commissions essentially no new art.
...
## Story data model (the authoring contract)

A story is pure data — text plus references into the content library. Adding a story means writing one JSON file against this shape; `StoryValidator` then confirms every word is decodable-or-whitelisted and every asset reference resolves. No new art, no code.
...
`StoryValidator` rejects a story unless: every non-`glue` word is in the content library's decodable inventory; every `image`/`choice`/`cast` ref resolves to a real asset; each question's `answer` is one of its `choices`; and there are ≥2 question sets and 2–3 beats.
...
- Modern stack: **`@Observable`** for view models, **SwiftData** for persistence, modern SwiftUI animation APIs for the calm transitions and the sequencing replay.
- **Audio** via **`AVAudioPlayer`** with pre-recorded human-voice clips; **`AVSpeechSynthesizer`** is the swappable dev placeholder so the loop is testable from day one.
- **Persistence** via **SwiftData** (active stories, parent settings, per-child mastery — all on device).
- **Zero third-party dependencies. Fully offline.** No ads, no in-app purchases, no network, no accounts, no analytics.
- **Parent gate**: press-and-hold then drag to reveal settings — never a one-tap "are you a grown-up?" dialog.
- **Project generation:** there is no Xcode GUI in the build environment — scaffold the project with **XcodeGen** (a checked-in `project.yml`) or a Swift Package executable/app target, so the whole thing builds headless with `xcodebuild`.

---

## Build plan / MVP

Ordered first-build cut. The guiding principle: **invest in the authoring template before content, and ship the sequencing game early.**

1. **Authoring template + fixed cast.** Define the `Story` data model (lines, per-word glue/decodable flags, question sets keyed to picture-answer choices, ordered beats) and the JSON authoring format above. Build `StoryValidator`. Wire in the **fixed reusable cast** (the cat, the dog, the pig, the boy) from the content library. *This is step one because content is the long pole.*
2. **Narrated Story Player.** Render an illustrated story with **word-by-word highlighting** driven by the audio engine, a large replay control, and **tap-a-word-to-hear-it**. Use `AVSpeechSynthesizer` as the placeholder voice.
3. **Spoken questions + picture-answer + errorless feedback.** The *who / what / where* flow with 2–4 picture choices, warm-confirm-and-expand on hit, gentle re-model-and-retry on miss.
4. **Reread with a new question set.** Replay the same story, draw the next question set.
5. **Sequencing drag-game (ship early).** Drag 2–3 beat cards into order, then play the assembled animation. Cheap, and it carries the memory pillar.
6. **Wordless Story Shelf + parent gate + settings.**
7. **Five authored micro-stories** using the template and fixed cast, each with two question sets and a beat ordering.

**Deferred (post-MVP):**

- Pre-recorded **human-voice** clips replacing the synthesizer placeholder (the loop ships testable without them).
- Deeper **review-service** integration (resurfacing stories whose vocabulary is due).
- More cast members and a larger story library beyond the first five.
- A parent-facing **authoring UI** (the MVP authors via the JSON template — no in-app editor).
- *Why*-style inference questions and richer multi-scene stories.

---

## Build & run

No Xcode GUI is required. Generate the project, build for an iPad simulator, and launch it headless:
...
Simulator/scheme names and the bundle id are placeholders — match them to what `project.yml` actually defines.

---

## Acceptance criteria

- [ ] A child can complete the **entire core loop** — listen → answer 2–3 spoken questions by tapping pictures → reread with new questions → drag the story into order → watch the replay — **without reading anything and without any way to fail**.
- [ ] **Every instruction is spoken.** No child-facing screen requires reading. The only readable text is behind the parent gate.
- [ ] **Errorless throughout:** a "wrong" tap never produces a buzzer or fail screen; it re-models the correct answer and allows a retry. Correct answers are **confirmed and expanded** into a fuller sentence.
- [ ] **Word-by-word highlighting** tracks the narration, and **tapping any word** replays that word.
- [ ] The **sequencing game** works with drag-and-drop on touch, accepts any order, and replays the story as an animation built from the child's ordering.
- [ ] **Calm constraints hold:** muted palette, soft transitions, one warm voice, and **no** confetti, streaks, timers, or visible scores.
- [ ] A new story can be added by **writing 3–4 decodable lines + a question set + a beat ordering** in the authoring template — **no new art and no code changes** — and `StoryValidator` confirms decodability and asset references.
- [ ] At least **five micro-stories** ship, each with two question sets and a beat ordering, all using the fixed reusable cast.
- [ ] **Parent gate** (press-and-hold then drag) hides all settings from the child.
- [ ] The reusable engines (**content library, audio engine, review service**) have **no app-specific imports** and are structured for extraction into (or reuse from) `LearningKit`.
- [ ] The **audio engine** plays `AVAudioPlayer` clips and falls back to `AVSpeechSynthesizer` as a swappable placeholder.
- [ ] **Fully offline:** no network calls, no analytics, no accounts; persistence is SwiftData on-device; **zero third-party dependencies**.
- [ ] **Builds clean with `xcodebuild` and runs on an iPad simulator** (iPadOS 17+), **landscape-locked** — and the core loop is verified by actually driving it, not just by compiling.
